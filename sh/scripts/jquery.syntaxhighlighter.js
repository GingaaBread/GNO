 ...
 * @copyright (c) 2009+ Benjamin Arthur Lupton {@link https://balupton.com}
...
if (typeof window.console.emulated === 'undefined') {
	...
	if (typeof window.console.debug === 'function') {
		...
		if (typeof window.console.debug === 'undefined') {
			window.console.debug = !window.console.hasLog
				? function() {}
				: function() {
	...
	if (typeof window.console.warn === 'function') {
		...
		if (typeof window.console.warn === 'undefined') {
			window.console.warn = !window.console.hasLog
				? function() {}
				: function() {
		...
		if (typeof window.console.error === 'undefined') {
			window.console.error = function() {
				...
				if (window.console.hasLog) {
					...
					// Adjust Message
					msg =
						"An error has occured. More information is available in your browser's javascript console."
				}
 ...
 * @package jquery-syntaxhighlighter {@link https://balupton.com/projects/jquery-syntaxhighlighter}
 */
			...
			highlight: function(params) {
				...
				if (!SyntaxHighlighter.loaded()) {
					if (config.debug)
						window.console.debug(
							'SyntaxHighlighter.highlight: Chosen SyntaxHighlighter is not yet defined. Waiting 1200 ms then trying again.'
						)
					setTimeout(function() {
				...
				if (defaultSelector === '.' || !defaultClassname) {
					window.console.error(
						'SyntaxHighlighter.highlight: Invalid defaultClassname.',
						[this, arguments],
				...
				// Fetch
				var $codes = $el
					.findAndSelf('code,pre')
					.filter('[class*=lang],' + defaultSelector)
					.filter(':not(.prettyprint)')
				...
				$codes
					...
					.addClass('prettyprint ' + defaultClassname)
					.each(function() {
						// Prepare
						var $code = $(this),
							css = $code.attr('class'),
				...
				if (config.stripEmptyStartFinishLines) {
					$codes
						.find('li:first-child > :first-child, li:last-child > :first-child')
						.each(function() {
							// Prepare
